...
namespace YGCLE
{
	public partial class Form1 : Form
	{
		...
		public int[] TransposeArrayDown1 = new int[] { 1, 2, 2, 3, 3, 4, 5, 5, 6, 6, 7, 7 };
		public int[] TransposeArrayDown2 = new int[] { 3, 2, 3, 2, 3, 3, 2, 3, 2, 3, 2, 3 };

		public int[] TransposeArrayUp1 = new int[] { 1, 1, 2, 2, 3, 4, 4, 5, 5, 6, 6, 7 };
		public int[] TransposeArrayUp2 = new int[] { 3, 4, 3, 4, 3, 3, 4, 3, 4, 3, 4, 3 };
		...
		public Form1()
		{
			...
			this.radioButton_file_chord.BackColor = Color.FromArgb(255, 255, 191);
			this.radioButton_file_length.BackColor = Color.FromArgb(255, 255, 191);
			...
			InitChordBuffer(780, 30, 5, 7, 70, 30);
			InitChordGrid(10, 280, 16, 8, 70, 30);
		}
		...
		public void InitChordGrid(int PositionX, int PositionY, int CountX, int CountY, int SizeX, int SizeY)
		{
			...
			for (int I = 0; I < this.ChordGridLabel.Length; I++)
			{
				...
				this.ChordGridLabel[I].MouseClick += delegate(object sender, MouseEventArgs e)
				{
					...
					if (e.Button == MouseButtons.Left)
					{
						if (this.radioButton_file_chord.Checked)
						{
						...
						else if (this.radioButton_file_length.Checked)
						{
							this.Length = Index + 1;
		...
		{
			e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(255, 191, 191)), 0, 20, 300, 230);
			e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(255, 255, 191)), 300, 20, 540, 230);
			e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(191, 255, 191)), 770, 20, 370, 230);
			e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(191, 255, 255)), 0, 250, 1140, 280);

			e.Graphics.DrawLine(new Pen(Color.FromArgb(0, 0, 0)), 0, 20, this.ClientSize.Width - 1, 20);
			e.Graphics.DrawLine(new Pen(Color.FromArgb(0, 0, 0)), 0, 250, this.ClientSize.Width - 1, 250);
			e.Graphics.DrawLine(new Pen(Color.FromArgb(0, 0, 0)), 0, 270, this.ClientSize.Width - 1, 270);
			e.Graphics.DrawLine(new Pen(Color.FromArgb(0, 0, 0)), 300, 0, 300, 250);
			e.Graphics.DrawLine(new Pen(Color.FromArgb(0, 0, 0)), 770, 0, 770, 250);
		}
		...
		{
			this.radioButton_file_chord.Checked = true;
			this.radioButton_file_length.Checked = false;
		...
		public void button_file_pitchdown_Click(object sender, EventArgs e)
		{
			for (int I = 0; I < this.Length; I++)
			{
				...
				if (this.comboBox_file_option.SelectedIndex == 0)
				{
					...
					if (Index < 0) { Index = 11; }
					if (Index > 11) { Index = 0; }
					SetPitch(I, this.TransposeArrayDown1[Index], this.TransposeArrayDown2[Index]);
				}
				else
				{
					int Index = GetPitch(this.ChordGridData[I].Value1, this.ChordGridData[I].Value2) - 1;
					if (Index < 0) { Index = 11; }
					if (Index > 11) { Index = 0; }
					SetPitch(I, this.TransposeArrayUp1[Index], this.TransposeArrayUp2[Index]);
				}
			}
			ChordGridToGui();
	}
		...
		public void CldToGui()
		{
			...
			for (int I1 = 0; I1 < this.ChordGridData.Length; I1++)
			{
				if (this.ChordGridData[I1] == null) { continue; }

				bool Flag = false;
				for (int I2 = 0; I2 < this.BufferData.Length; I2++)
				{
					if (this.BufferData[I2] == null) { continue; }

					int Value = 0;
					if (this.BufferData[I2].Value1 == this.ChordGridData[I1].Value1) { Value = Value + 1; }
					if (this.BufferData[I2].Value2 == this.ChordGridData[I1].Value2) { Value = Value + 1; }
					if (this.BufferData[I2].Value3 == this.ChordGridData[I1].Value3) { Value = Value + 1; }
					if (Value == 3) { Flag = true; break; }
				}
				...
				if (!Flag)
				{
					for (int I2 = 0; I2 < this.BufferData.Length; I2++)
					{
						if (this.BufferData[I2] == null)
						{
							this.BufferData[I2] = this.ChordGridData[I1];
							break;
						}
					}
				}
			...
			for (int I = 0; I < this.BufferData.Length; I++)
			{
				if (this.BufferData[I] != null)
				{
					if (this.BufferData[I].Value3 == 34) { this.BufferLabel[I].Text = this.listBox_type.Items[this.BufferData[I].Value3].ToString(); }
					{ this.BufferLabel[I].Text = this.listBox_notelo.Items[this.BufferData[I].Value1].ToString() + this.listBox_notehi.Items[this.BufferData[I].Value2].ToString() + this.listBox_type.Items[this.BufferData[I].Value3].ToString(); }
				}
			}
		...
		public class CLD
		{
			...
			public class Util
			{
				...
				{
					byte Byte1 = (byte)(Value % 256);
					byte Byte2 = (byte)(Value / 256 % 256);
				...
				public static uint SwapUInt32(uint Value)
				{
					byte Byte1 = (byte)(Value % 256);
					byte Byte2 = (byte)(Value / 256 % 256);
					byte Byte3 = (byte)(Value / 65536 % 256);
					byte Byte4 = (byte)(Value / 16777216 % 256);
